...
public class FB_UserInfo {
    ...
    public FB_UserInfo(JSONNode JSON) {
        InitializeData(JSON);
    }

    public void InitializeData(JSONNode pJSON) {
        _id = pJSON["id"].Value;
        _name = pJSON["name"].Value;
        _first_name = pJSON["first_name"].Value;
        _last_name = pJSON["last_name"].Value;
        _picUrl = pJSON["picture"]["data"]["url"].Value;
    }
